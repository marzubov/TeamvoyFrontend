  ...
  function Draggable(grid) {
    ...
    function swapTemplates(firstCol, secondCol) {
      ...
      if (countWithTemplates == 1) {
        if (whatColumn == firstCol) {
          grid.config.columnTemplates.renameProperty(firstCol, secondCol);
        }
        else {
          grid.config.columnTemplates.renameProperty(getKeyByIndexColumn(secondCol), gfirstCol);
        }
      }
